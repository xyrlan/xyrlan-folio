    ...
    return (
        <nav className={`sticky top-0 bg-slate-100 flex w-full lg:p-5 lg:px-10  justify-center border-b-2 z-50 overflow-hidden ${!isActive ? '' : ''}`}>
            <div className="container flex w-full lg:p-5 lg:px-10 justify-between ">
                ...
                {isActive && (
                    <motion.ul initial={{ height: 0 }} whileInView={{ height: '100%', opacity: 1 }} transition={{ duration: 1 }} className={`fixed bg-slate-100 h-full w-full flex flex-col justify-start pt-32 items-center gap-10 z-30 top-0`}>
